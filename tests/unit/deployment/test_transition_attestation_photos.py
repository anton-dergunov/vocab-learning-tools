"""The one-off converter that added `attestations.photo_ref` and `attestations.photo_region`.

**Delete this file together with `scripts/throwaway/add_attestation_photos.py`**, once that has run.
It is tested against a database built the way the owner's was — the current schema minus the two
...
_spec = importlib.util.spec_from_file_location(
    "add_attestation_photos", ROOT / "scripts" / "throwaway" / "add_attestation_photos.py")
...
def old(tmp_path) -> Path:
    """The owner's database as it was: the current schema without the columns, one attestation."""
    path = tmp_path / "acervo.db"
    engine = create_engine(f"sqlite+pysqlite:///{path}")
    metadata.create_all(engine)
    lexeme = _row(metadata.tables["lexemes"], id="lexeme000000001", owner="owner0000000001")
    row = _row(metadata.tables[converter.TABLE], id="attestation0001", owner="owner0000000001",
               lexeme="lexeme000000001", text="Se pican las verduras.")
    with engine.begin() as connection:
        connection.execute(text("PRAGMA foreign_keys = OFF"))
        connection.execute(text(
            f"INSERT INTO lexemes ({', '.join(lexeme)}) VALUES ({', '.join(':' + k for k in lexeme)})"),
            lexeme)
...
def test_the_head_it_converts_to_is_not_the_one_it_converts_from():
    """The script is only needed because the schema moved; if this fails, the columns went away."""
    assert HEAD != converter.FROM_REVISION
...
def test_it_converts_from_the_head_it_was_written_for_and_keeps_every_row(old):
    ...
    rows = _query(old, f"SELECT id, text, photo_ref, photo_region FROM {converter.TABLE}")
    assert rows == [("attestation0001", "Se pican las verduras.", "", None)], (
        "what was there survives, and reads as an attestation with no photo"
    )
...
def test_a_dry_run_checks_everything_and_writes_nothing(old, capsys):
    ...
    columns = {row[1] for row in _query(old, f"PRAGMA table_info({converter.TABLE})")}
    assert not columns & set(converter.COLUMNS)
...
def test_a_run_that_added_one_column_and_stopped_can_be_finished(old):
    """The columns and the stamp are three statements; a crash between them must not strand the
    database on a converter that then refuses because a column is already there."""
    engine = create_engine(f"sqlite+pysqlite:///{old}")
    with engine.begin() as connection:
        connection.execute(text(
            f"ALTER TABLE {converter.TABLE} ADD COLUMN photo_ref {converter.COLUMNS['photo_ref']}"))
    engine.dispose()
    assert _stamp(old) == converter.FROM_REVISION
...
def test_it_refuses_when_anything_but_those_two_columns_has_moved(old, capsys):
