...
It carries one Acervo database across one specific schema change — the one that gave `story_words` a
`translation_forms` column, so a story's translation can mark the word it teaches — and it exists so
...
additive*: one new column with a default, and not one existing column, index or constraint altered.
...
anything but that one column has moved. If that check fails, this script is the wrong tool and
...
have taken first, with the server stopped:
    ...
    python scripts/throwaway/add_story_translation_forms.py --database PATH --dry-run
"""
...
FROM_REVISION = "bootstrap_3e2e895039db"

TABLE = "story_words"
COLUMN = "translation_forms"
...
def main(argv: list[str] | None = None) -> int:
    ...
    missing = f"{TABLE}: column {COLUMN!r} is in the code but not the database"
    ...
    # the column a second time would fail rather than finish the job.
    if changed not in ([missing], []):
        problems.extend(change for change in changed if change != missing)
    if problems:
        print(
            "This change is only safe because nothing but one column was added, and something else moved:\n  "
            + "\n  ".join(problems)
    ...
    add_column = changed == [missing]
    print(f"{len(others)} other tables match the code exactly.")
    print(f"Adding: {TABLE}.{COLUMN}" if add_column else f"{TABLE}.{COLUMN} is already there.")
    ...
    with engine.begin() as connection:
        if add_column:
            # An explicit DDL default, because SQLAlchemy's `default=` is applied in Python and
            # SQLite refuses to add a NOT NULL column that has none. Every existing row reads `[]`,
            # which is what "the translator did not say" means.
            connection.execute(text(f"ALTER TABLE {TABLE} ADD COLUMN {COLUMN} JSON NOT NULL DEFAULT '[]'"))
