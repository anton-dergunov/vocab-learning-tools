...
It carries one Acervo database across one specific schema change — the one that gave `story_parts`
the columns a recording of the part is kept in (`audio_ref` and the five beside it) — and it exists so
that the owner's words, pictures, recordings and stories are not rebuilt along with the schema.
...
additive*: six new columns with defaults on one table, and not one existing column, index or
constraint altered.
...
anything but those columns has moved. If that check fails, this script is the wrong tool and
...
have taken first, with the server stopped:
    ...
    python scripts/throwaway/add_story_part_audio.py --database PATH --dry-run
"""
...
FROM_REVISION = "bootstrap_b6c8f84cdff7"

TABLE = "story_parts"
# An explicit DDL default on each, because SQLAlchemy's `default=` is applied in Python and SQLite
# refuses to add a NOT NULL column that has none. Every existing part reads as "not recorded".
COLUMNS = {
    "audio_ref": "VARCHAR(500) NOT NULL DEFAULT ''",
    "audio_mime": "VARCHAR(120) NOT NULL DEFAULT ''",
    "audio_provider_id": "VARCHAR(120) NOT NULL DEFAULT ''",
    "audio_model_id": "VARCHAR(120) NOT NULL DEFAULT ''",
    "audio_voice": "VARCHAR(120) NOT NULL DEFAULT ''",
    "audio_segments": "JSON NOT NULL DEFAULT '[]'",
}
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Two states are acceptable. Either the columns are the one thing missing, or they are already
    # there and only the stamp did not move — which is what an interrupted earlier run leaves, and
    # adding them a second time would fail rather than finish the job.
    if sorted(changed) not in (sorted(missing), []):
        problems.extend(change for change in changed if change not in missing)
    if problems:
        print(
            "This change is only safe because nothing but those columns was added, and something else moved:\n  "
            + "\n  ".join(problems)
    ...
    print(f"Adding: {TABLE}.({', '.join(COLUMNS)})" if add_columns else f"{TABLE}'s columns are already there.")
