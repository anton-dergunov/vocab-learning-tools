...
It carries one Acervo database across one specific schema change — the one that added `beds`, the
music beds the owner keeps as favourites — and it exists so that the owner's words, pictures,
...
have taken first, with the server stopped:
    ...
    python scripts/throwaway/add_beds.py --database PATH --dry-run
"""
...
FROM_REVISION = "bootstrap_042af91e4b05"
