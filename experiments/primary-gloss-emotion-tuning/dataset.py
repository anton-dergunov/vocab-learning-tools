...
than a copy of it — the same rule `experiments/compose-lesson-line/dataset.py` lives by, adapted here
for a word list that mixes real production headwords with synthetic collocations.

Resolve is deliberately skipped, exactly as in `compose-lesson-line`: `words.yaml` states the
resolution — headword, lemma, part of speech, language, the learner's sentences — because resolve is
a separate prompt this experiment does not vary.
"""
...
def words(only: list[str] | None = None, category: str | None = None) -> list[dict[str, Any]]:
    rows = load()["words"]
    if category:
