...
| [`primary-gloss-emotion-tuning/`](primary-gloss-emotion-tuning/README.md) | `primaryGloss` and `emotion` shipped and don't thin the article, but are they worded well? Is `primaryGloss` too short for a multi-word headword, and is `emotion` null too often? | Measured 21 Sep 2026: `primaryGloss` auto-fail rate 15.6% → 4.4% (0% on a holdout); `emotion` coverage on ordinary words 57–89% → 88.6–100%, with 0% false positives on a null-word control after one corrective round. Wording shipped; the real production null rate is still unmeasured (data-access gap, see the README) | [`docs/plans/article-quality.md`](../docs/plans/article-quality.md) |
